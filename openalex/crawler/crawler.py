...
def main():
    parser = argparse.ArgumentParser(
        description="Extract target Work IDs from an OpenAlex snapshot."
    )
    parser.add_argument(
        "openalex_works_folder", help="Path to the openalex-snapshot 'works' folder"
    )
    parser.add_argument(
        "openalex_csv_file",
        help="Path to the openalex csv file containing target Work IDs",
    )
    parser.add_argument(
        "--max-workers",
        type=int,
    ...
    print(
        f"Created file with {len(get_only_false_work_ids(_work_ids))} entries for work ids not found in openalex-snapshot."
    )
